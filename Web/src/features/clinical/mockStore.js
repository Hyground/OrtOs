...
const mariaCharge = {
  id: 'charge-maria-consulta', patientId: 'patient-1', appointmentId: 'appointment-1',
  description: 'Consulta general', quantity: 1, unitPrice: 150, subtotal: 150,
  date: '2026-09-21', status: 'Parcial', allocations: [{ paymentId: 'payment-1', amount: 100 }],
}
const mariaPlan = {
  id: 'plan-maria-ortodoncia', patientId: 'patient-1', name: 'Ortodoncia', totalAmount: 2400,
  downPayment: 600, financedAmount: 1800, installmentCount: 6, status: 'Activo',
  installments: [1, 2, 3, 4, 5, 6].map((number) => ({ number, amount: 300, dueDate: `2026-${String(6 + number).padStart(2, '0')}-15`, paidAmount: number <= 2 ? 300 : 0, status: number <= 2 ? 'Pagada' : 'Pendiente' })),
}
let state = {
  patients: patientSeed,
  appointments: appointmentSeed,
  payments: paymentSeed.map((payment) => payment.id === 'payment-1' ? { ...payment, patientId: 'patient-1', concept: 'Anticipo', treatment: 'Consulta general', date: '2026-09-21', amount: 100, method: 'Efectivo', reference: 'REC-001', allocations: [{ chargeId: mariaCharge.id, amount: 100 }] } : payment),
  charges: [mariaCharge], paymentPlans: [mariaPlan],
}
...
export async function addCharge(data) {
  ...
  const charge = { ...data, id: 'charge-' + crypto.randomUUID(), quantity, unitPrice, subtotal: Number((quantity * unitPrice).toFixed(2)), date: localDate(), status: 'Pendiente', allocations: [] }
...
export async function registerAccountPayment(data) {
  ...
  const pending = state.charges.filter((c) => c.patientId === data.patientId && c.status !== 'Anulado' && c.subtotal > paidFor(c))
  ...
  const updated = state.charges.map((charge) => {
    ...
    const paid = paidFor(charge); const applied = Math.min(remaining, charge.subtotal - paid); remaining -= applied
    const chargeAllocations = [...(charge.allocations || []), { paymentId, amount: applied }]; const total = paid + applied
    allocations.push({ chargeId: charge.id, amount: applied })
    return { ...charge, allocations: chargeAllocations, status: total === charge.subtotal ? 'Pagado' : 'Parcial' }
  })
  if (remaining > 0) throw new Error('El abono supera los cargos seleccionados.')
  const payment = { ...data, id: paymentId, concept: 'Abono a cuenta', treatment: '', date: localDate(), currency: 'GTQ', receipt: true, receiptNumber: nextReceipt(), status: 'Completado', allocations }
  state = { ...state, charges: updated, payments: [payment, ...state.payments] }; listeners.forEach((fn) => fn()); return payment
}
export const accountFor = (patientId, appointmentId = '') => {
  ...
  const paid = charges.reduce((s, c) => s + (c.allocations || []).reduce((a, n) => a + Number(n.amount ?? n), 0), 0)
  const paymentIds = new Set(charges.flatMap((c) => (c.allocations || []).map((a) => a.paymentId)).filter(Boolean))
  return { charges, payments: state.payments.filter((p) => paymentIds.has(p.id)), total, paid, balance: Math.max(0, total - paid) }
}
export async function createPaymentPlan({ patientId, totalAmount, downPayment = 0, financedAmount, installmentCount, firstDueDate, name = 'Plan de pago' }) {
  ...
  const start = firstDueDate || new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1).toISOString().slice(0, 10)
  const installments = Array.from({ length: count }, (_, index) => ({ number: index + 1, amount: index === count - 1 ? Number((amount - base * (count - 1)).toFixed(2)) : base, dueDate: new Date(new Date(start + 'T12:00:00').getFullYear(), new Date(start + 'T12:00:00').getMonth() + index, new Date(start + 'T12:00:00').getDate()).toISOString().slice(0, 10), paidAmount: 0, status: 'Pendiente' }))
  const plan = { id: 'plan-' + crypto.randomUUID(), patientId, name, totalAmount: Number(totalAmount || amount), downPayment: Number(downPayment), financedAmount: amount, installmentCount: count, status: 'Activo', installments }
