...
import { accountFor, addCharge, createPaymentPlan, displayDate, money, registerAccountPayment, useClinic } from '@/features/clinical/mockStore'
...
export function PatientAccount({ patient, appointment, compact = false }) {
  ...
  return <section className={[css.account, compact ? css.compactAccount : ''].filter(Boolean).join(' ')}>
    ...
    <div className={css.accountKpis}>
      <Metric label="Facturado" value={money(account.total)} />
      <Metric label="Pagado" value={money(account.paid)} tone="positive" />
      <Metric label="Pendiente" value={money(account.balance)} tone={account.balance ? 'warning' : 'positive'} />
      <Metric label="Próximo pago" value={nextInstallment ? money(nextInstallment.amount) : '—'} detail={nextInstallment ? displayDate(nextInstallment.dueDate) : 'Sin cuotas pendientes'} />
    </div>
...
function AccountSummary({ account, pending, onCharge, onPayment }) { return <div className={css.summaryGrid}><section className={css.balanceCard}><span>CUENTA ACTUAL</span><div><small>Total cargos</small><strong>{money(account.total)}</strong></div><div><small>Pagado</small><strong>{money(account.paid)}</strong></div><hr /><div className={css.balanceTotal}><small>SALDO</small><strong>{money(account.balance)}</strong></div>{!account.balance && <p>✓ Sin pagos pendientes</p>}</section><section className={css.summaryActions}><h3>{pending.length ? `${pending.length} cargos pendientes` : 'Cuenta al día'}</h3><p>{pending.length ? 'Registra un abono o agrega cargos de una cita.' : 'No hay saldos pendientes para este paciente.'}</p><Button onClick={onPayment} disabled={!account.balance}>Registrar pago</Button><Button variant="ghost" onClick={onCharge}>+ Agregar cargo</Button></section></div> }
function ChargeList({ charges, appointments, onAdd }) { const appointmentFor = (id) => appointments.find((item) => item.id === id); return <><div className={css.sectionHeading}><div><h3>CARGOS DE LA CUENTA</h3><p>Conceptos cobrados al paciente y su aplicación.</p></div><Button size="sm" onClick={onAdd}>+ Agregar cargo</Button></div><div className={css.ledger}>{charges.map((charge) => { const related = appointmentFor(charge.appointmentId); const paid = paidFor(charge); return <article className={css.chargeRow} key={charge.id}><div><strong>{charge.description}</strong><small>{displayDate(charge.date)} · {related ? `${related.treatment} · ${related.dentist}` : 'Sin cita relacionada'}</small></div><span>{charge.quantity} × {money(charge.unitPrice)}</span><strong>{money(charge.subtotal)}</strong><span>{money(paid)}</span><Badge>{chargeStatus(charge)}</Badge></article> })}{!charges.length && <p className={css.empty}>Aún no hay cargos para esta cuenta.</p>}</div></> }
function PaymentList({ payments, onAdd }) { return <><div className={css.sectionHeading}><div><h3>PAGOS RECIBIDOS</h3><p>Solo dinero realmente recibido y aplicado.</p></div><Button size="sm" onClick={onAdd}>+ Registrar pago</Button></div><div className={css.ledger}>{payments.map((payment) => <article className={css.paymentRow} key={payment.id}><div><strong>{displayDate(payment.date)}</strong><small>{payment.reference || payment.receiptNumber || 'Pago en clínica'}</small></div><span>{payment.method}</span><strong>{money(payment.amount)}</strong><Badge>{payment.status}</Badge></article>)}{!payments.length && <p className={css.empty}>No hay pagos aplicados todavía.</p>}</div></> }
function PendingList({ charges, appointments }) { const appointmentFor = (id) => appointments.find((item) => item.id === id); return <div className={css.pendingList}>{charges.map((charge) => { const paid = paidFor(charge); const related = appointmentFor(charge.appointmentId); return <article className={css.pendingCard} key={charge.id}><div><Badge>{chargeStatus(charge)}</Badge><h3>{charge.description}</h3><small>{related ? `Cita ${displayDate(related.date)}` : 'Cuenta general'}</small></div><dl><div><dt>Total</dt><dd>{money(charge.subtotal)}</dd></div><div><dt>Pagado</dt><dd>{money(paid)}</dd></div><div><dt>Pendiente</dt><dd>{money(charge.subtotal - paid)}</dd></div></dl></article> })}{!charges.length && <p className={css.empty}>No existen pendientes.</p>}</div> }
function Plans({ plans, onAdd }) { return <><div className={css.sectionHeading}><div><h3>PLANES DE PAGO</h3><p>Financiamiento del saldo sin duplicar cargos.</p></div><Button size="sm" onClick={onAdd}>+ Crear plan de pago</Button></div><div className={css.planList}>{plans.map((plan) => { const paid = plan.installments.reduce((sum, installment) => sum + installment.paidAmount, 0); const progress = plan.financedAmount ? Math.min(100, paid / plan.financedAmount * 100) : 0; return <article className={css.planCard} key={plan.id}><header><div><Badge>{plan.status}</Badge><h3>{plan.name}</h3><small>Total {money(plan.totalAmount)} · Anticipo {money(plan.downPayment)}</small></div><strong>{plan.installmentCount} cuotas de {money(plan.financedAmount / plan.installmentCount)}</strong></header><div className={css.progress}><span style={{ width: progress + '%' }} /></div><p>Pagado: <b>{money(paid)}</b> / {money(plan.financedAmount)}</p><div className={css.installments}>{plan.installments.map((installment) => <span key={installment.number}>#{installment.number} · {money(installment.amount)} <Badge>{installment.status}</Badge></span>)}</div></article> })}{!plans.length && <p className={css.empty}>No hay planes de pago activos.</p>}</div></> }
